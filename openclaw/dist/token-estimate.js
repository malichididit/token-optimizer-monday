...
exports.CODE_CHARS_PER_TOKEN = void 0;
exports.estimateTokens = estimateTokens;
exports.estimateTokensFromBytes = estimateTokensFromBytes;
exports.CODE_CHARS_PER_TOKEN = 3.3;
function isCjk(code) {
...
function estimateTokens(text) {
    ...
    if (!NON_ASCII.test(text)) {
        return Math.max(1, Math.ceil(text.length / exports.CODE_CHARS_PER_TOKEN));
    }
    let cjk = 0;
    ...
    return Math.max(1, Math.ceil(other / exports.CODE_CHARS_PER_TOKEN) + cjk);
}
/** Estimate tokens from a byte count (e.g. fs.statSync().size). Assumes ~ASCII. */
function estimateTokensFromBytes(nBytes) {
    if (!nBytes || nBytes <= 0)
        return 0;
    return Math.max(1, Math.ceil(nBytes / exports.CODE_CHARS_PER_TOKEN));
}
