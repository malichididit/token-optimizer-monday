...
exports.generateCoachData = generateCoachData;
...
function dominantContextWindow(sessions) {
    ...
    return (0, quality_1.contextWindowForModel)(best);
}
// ---------------------------------------------------------------------------
// Main generator
// ---------------------------------------------------------------------------
 ...
 */
function generateCoachData(audit, sessions, costlyPrompts, agentCosts, unusedSkillFindings) {
    ...
    const patternsBad = [];
    // --- SOUL.md analysis ---
    const soulComponent = audit.components.find((c) => c.name === "SOUL.md");
    ...
    if (unusedSkillFindings.length > 0) {
        ...
        const unusedCount = finding.evidence?.unusedCount ?? 0;
        const ratio = (finding.evidence?.unusedRatioPct ?? 0) / 100;
    ...
    if (sessions.length >= 5) {
        const expensiveCount = sessions.filter((r) => models_1.EXPENSIVE_MODELS.has(r.model)).length;
